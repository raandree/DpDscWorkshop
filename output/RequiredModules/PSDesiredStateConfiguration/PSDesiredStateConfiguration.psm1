...
function Configuration
{
    ...
    try
    {
        ...
        if ($Script:PSConfigurationName -eq '')
        {
            ...
            $sb = [Microsoft.PowerShell.DesiredStateConfiguration.Internal.DscClassCache].GetField('_cimKeywordImlementationFunction', 'NonPublic, Static').GetValue($null)
            $hash = [System.BitConverter]::ToString([System.Security.Cryptography.HashAlgorithm]::Create("sha256").ComputeHash([System.Text.Encoding]::UTF8.GetBytes($sb.ToString())))
            if ($hash -eq '24-A8-67-2C-42-5D-A2-F7-5E-4C-C5-DD-D8-9B-BC-DC-D3-F3-9B-DB-4B-BE-16-F5-1C-8F-B1-E5-CC-F0-7E-BC') {
                [Microsoft.PowerShell.DesiredStateConfiguration.Internal.DscClassCache].GetField('_cimKeywordImlementationFunction', 'NonPublic, Static').SetValue($null, [scriptblock]::Create($cimKeywordImplementationFunctionText))
            }
            ...
            $newDependsOn = foreach ($requiredResource in $Script:NodeResources[$resourceId])
            {
                ...
                if ($requiredResource)
                {
                    ...
                    if (-not $Script:NodeResources.ContainsKey($requiredResource))
                    {
                        ...
                        {
                            $rr = if ($requiredResource.Split('::').Count -gt 1) {
                                $requiredResource.Substring(0, $requiredResource.LastIndexOf('::')).Replace('\\', '\')
                            }
                            else {
                                $requiredResource
                            }
                            if($rId.EndsWith($rr, [System.StringComparison]::InvariantCultureIgnoreCase))
                            {
                                $rId
                            }
                        }
...
$cimKeywordImplementationFunctionText = @'
param (
    [Parameter(Mandatory)]
    $KeywordData,
    [Parameter(Mandatory)]
...
# walk the call stack to get at all of the enclosing configuration resource IDs
$stackedConfigs = @(Get-PSCallStack | Where-Object { ($null -ne $_.InvocationInfo.MyCommand) -and ($_.InvocationInfo.MyCommand.CommandType -eq 'Configuration') })
# keep all but the top-most
...
function Test-DependsOn {
    ...
    $updatedDependsOn = foreach ($DependsOnVar in $value['DependsOn']) {
        ...
        if ($DependsOnVar -notmatch '^\[[a-z]\w*\][a-z_0-9][a-z_0-9\p{Zs}\.\\-]*(::\[[a-z]\w*\][a-z_0-9][a-z_0-9\p{Zs}\.\\-]*)?$') {
            Update-ConfigurationErrorCount
...
if (($keywordData.Properties.Keys -contains 'DependsOn') -or (($KeywordData.ImplementingModule -ieq 'PSDesiredStateConfigurationEngine') -and ($KeywordData.NameMode -eq [System.Management.Automation.Language.DynamicKeywordNameMode]::NameRequired))) {
    ...
    if (Test-NodeResources $resourceId) {
        ...
        # Save the resource id in a per-node dictionary to do cross validation at the end
        if ($keywordData.ImplementingModule -eq 'PSDesiredStateConfigurationEngine') {
